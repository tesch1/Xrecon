...
#define SOURCEFILE "common/dhead.c"

void getdfh(struct data *d)
{
  char function[20];
  strcpy(function,"getdfh"); /* Set function name */
  ...
  if (d->buf.st_size == sizeof(d->fh)) { 
    fprintf(stdout,"\n%s: %s()\n",SOURCEFILE,function);
    fprintf(stdout,"  data file only contains a file header\n");
  }
  if (d->fh.bbytes > 0) {
...
void getdbh(struct data *d,int blockindex)
{
  ...
  strcpy(function,"getdbh"); /* Set function name */
  ...
  if (d->buf.st_size >= offset+sizeof(d->bh)) {
    ...
    fprintf(stdout,"\n%s: %s()\n",SOURCEFILE,function);
    fprintf(stdout,"  Problem reading block header %d in %s\n",
      blockindex,d->file);
  }
...
void gethcbh(struct data *d,int blockindex)
{
  long offset;
  char function[20];
  strcpy(function,"gethcbh"); /* Set function name */
  ...
  if ((d->fh.nbheaders > 1) 
    ...
    fprintf(stdout,"\n%s: %s()\n",SOURCEFILE,function);
    fprintf(stdout,"  Problem reading hypercomplex block header %d in %s\n",
      blockindex,d->file);
  }
