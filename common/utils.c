...
#define SOURCEFILE "common/utils.c"
...
void complexmatrixmultiply(gsl_matrix_complex *A,gsl_matrix_complex *B,gsl_matrix_complex *C)
{
  ...
  gsl_complex cx;
  char function[30];
  strcpy(function,"complexmatrixmultiply"); /* Set function name */

  /* Check Dimensions */
  if (B->size1 != A->size2) {
    fprintf(stdout,"\n%s: %s()\n",SOURCEFILE,function);
    fprintf(stdout,"  Input matrices (args 1&2) do not have compatible dimensions\n");
    fprintf(stdout,"  Aborting ...\n\n");
    return;
  }
  if ((C->size1 != A->size1) && (C->size2 != B->size2)) {
    fprintf(stdout,"\n%s: %s()\n",SOURCEFILE,function);
    fprintf(stdout,"  Resultant matrix (arg 3) does not have correct dimensions\n");
    fprintf(stdout,"  Aborting ...\n\n");
    return;
  }
...
int createdir(char *dirname)
{
  ...
  int type;
  char function[20];
  strcpy(function,"createdir"); /* Set function name */
  ...
  if (stat(dirname,&buf) == -1) { /* Check to see if it exists */
    if (mkdir(dirname, 00755) != 0) { /* If not then create */
      fprintf(stdout,"\n%s: %s()\n",SOURCEFILE,function);
      fprintf(stdout,"  Unable to create directory %s\n",dirname);
      return(1);
    }
    stat(dirname,&buf);
  }
...
int checkdir(char *dirname)
{
  ...
  int type;
  char function[20];
  strcpy(function,"checkdir"); /* Set function name */
...
int checkfile(char *filename)
{
  ...
  int type;
  char function[20];
  strcpy(function,"checkfile"); /* Set function name */
