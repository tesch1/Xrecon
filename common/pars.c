...
void getpars(char *procparfile,struct data *d)
{
  ...
  /* Set some data structure parameters accordingly */
  setdatapars(d);
...
void getprocparpars(char *procparfile,struct pars *p)
{
  ...
  int keepNext;
  char function[20];
  strcpy(function,"getprocparpars"); /* Set function name */
  ...
  while (fscanf(fp,"%s",name) != EOF) {
    ...
    if (fscanf(fp,"%*f %d %*f %*f %*f %*f %*f %*f %*f %*f %d",
      &type,&nvals) != 2)
      procparerror(function,name,"(attributes)");
    ...
    if (fscanf(fp,"%d",&nvals) != 1)
      procparerror(function,name,"(number of enumerable values)");
  ...
  for (i=0;i<p->npars;i++) {
    ...
    if ((p->name[i] = (char *)malloc((strlen(name)+1)*sizeof(char))) == NULL) nomem();
    ...
    switch(p->type[i]) {
      case 0:
        if ((p->i[i] = (int *)malloc(p->nvals[i]*sizeof(int *))) == NULL) nomem();
        for (j=0;j<p->nvals[i];j++) {
          if (fscanf(fp, "%d", &p->i[i][j]) != 1)
            procparerror(function,name,"(values)");
        }
        break;
      case 1:
        if ((p->d[i] = (double *)malloc(p->nvals[i]*sizeof(double *))) == NULL) nomem();
        for (j=0;j<p->nvals[i];j++) {
        ...
        break;
      case 2:
        if ((p->s[i] = (char **)malloc(p->nvals[i]*sizeof(char *))) == NULL) nomem();
        for (j=0;j<p->nvals[i];j++) {
          ...
          while (1) {
            ...
            if (nextChar == '"') {
              ...
              if ((p->s[i][j] = (char *)malloc((strlen(str)+1)*sizeof(char))) == NULL) nomem();
              strcpy(p->s[i][j],str);
              break;
            }
            else
...
#ifdef DEBUG
  char function[20];
  strcpy(function,"skipprocparvals"); /* Set function name */
#endif
...
{
  fprintf(stdout,"\n%s: %s()\n",SOURCEFILE,function);
  fprintf(stdout,"  Problem reading parameter '%s' %s\n",par,str);
  fflush(stdout);
}
...
void setarray(struct pars *p,struct pars *a)
{
...
#ifdef DEBUG
  char function[20];
  strcpy(function,"setarray"); /* Set function name */
#endif
  ...
  if ((a->d = (double **)malloc(sizeof(double *))) == NULL) nomem();
  if ((a->d[0] = (double *)malloc(*a->nvals*sizeof(double))) == NULL) nomem();
  /* Initialise a->d */
  ...
  */
  ...
  if ((cycles = (double *)malloc(*a->nvals*sizeof(double))) == NULL) nomem();
...
int list2array(char *par,struct pars *p1,struct pars *p2)
{
...
#ifdef DEBUG
  char function[20];
  strcpy(function,"list2array"); /* Set function name */
#endif
  ...
  *p2->nvals=1; /* there is at least one parameter in the list */
  ...
  /* Allocate for nvals */
  if ((p2->s = (char ***)malloc(sizeof(char **))) == NULL) nomem();
  if ((p2->s[0] = (char **)malloc(*p2->nvals*sizeof(char *))) == NULL) nomem();
  for (i=0;i<*p2->nvals;i++)
    if ((p2->s[0][i] = (char *)malloc((strlen(*sval(par,p1))+1)*sizeof(char))) == NULL) nomem();
...
void setval(struct pars *p,char *par,double value)
{
  ...
  if ((p2.name[0] = (char *)malloc((strlen(par)+1)*sizeof(char))) == NULL) nomem();
  strcpy(p2.name[0],par);
  /* Malloc and fill the parmaeter value */
  if ((p2.d[0] = (double *)malloc(sizeof(double *))) == NULL) nomem();
  *p2.d[0] = value;
...
void copypar(char *par,struct pars *p1,struct pars *p2)
{
  ...
  /* Find the index of par in the p1 set otherwise return */
  ix=parindex(par,p1);
  if (ix<0) return;

  /* Search for par in the p2 set to modify an exisiting parameter */
  for (i=0;i<p2->npars;i++) {
    if (!strcmp(p2->name[i],par)) { /* There is a match */
  ...
  for (i=0;i<p3.npars;i++) {
    if ((p3.name[i] = (char *)malloc((strlen(p2->name[i])+1)*sizeof(char))) == NULL) nomem();
  ...
  mallocnpars(p2);
  for (i=0;i<p3.npars;i++) {
    if ((p2->name[i] = (char *)malloc((strlen(p3.name[i])+1)*sizeof(char))) == NULL) nomem();
  ...
  if ((p2->name[i] = (char *)malloc((strlen(par)+1)*sizeof(char))) == NULL) nomem();
  strcpy(p2->name[i],par);
  /* Copy values from p1 to p2 */
  copyvalues(p1,ix,p2,i);
...
void copyvalues(struct pars *p1,int ix1,struct pars *p2,int ix2)
{
  ...
  switch(p2->type[ix2]) {
    case 0:
      if ((p2->i[ix2] = (int *)malloc(p2->nvals[ix2]*sizeof(int *))) == NULL) nomem();
      for (j=0;j<p2->nvals[ix2];j++) p2->i[ix2][j] = p1->i[ix1][j];
      break;
    case 1:
      if ((p2->d[ix2] = (double *)malloc(p2->nvals[ix2]*sizeof(double *))) == NULL) nomem();
      for (j=0;j<p2->nvals[ix2];j++) p2->d[ix2][j] = p1->d[ix1][j];
      break;
    case 2:
      if ((p2->s[ix2] = (char **)malloc(p2->nvals[ix2]*sizeof(char *))) == NULL) nomem();
      for (j=0;j<p2->nvals[ix2];j++) {
        if ((p2->s[ix2][j] = (char *)malloc((strlen(p1->s[ix1][j])+1)*sizeof(char))) == NULL) nomem();
...
{
  if ((p->name = (char **)malloc(p->npars*sizeof(char *))) == NULL) nomem();
  if ((p->type = malloc(p->npars*sizeof(int))) == NULL) nomem();
  if ((p->active = malloc(p->npars*sizeof(int))) == NULL) nomem();
  if ((p->nvals = malloc(p->npars*sizeof(int))) == NULL) nomem();
  if ((p->i = (int **)malloc(p->npars*sizeof(int *))) == NULL) nomem();
  if ((p->d = (double **)malloc(p->npars*sizeof(double *))) == NULL) nomem();
  if ((p->s = (char ***)malloc(p->npars*sizeof(char **))) == NULL) nomem();
}
...
int *ival(char *par,struct pars *p)
{
  ...
  for (i=0;i<p->npars;i++)
    if ((!strcmp(p->name[i],par)) && p->active[i] && (p->type[i] == 0))
      return(p->i[i]);
  if ((in = (int *)malloc(sizeof(int))) == NULL) nomem();
  *in=0;
  return(in);
}
...
double *val(char *par,struct pars *p)
{
  ...
  for (i=0;i<p->npars;i++)
    if ((!strcmp(p->name[i],par)) && p->active[i] && (p->type[i] == 1))
      return(p->d[i]);
  if ((d = (double *)malloc(sizeof(double))) == NULL) nomem();
  *d=0.0;
  return(d);
}
...
char **sval(char *par,struct pars *p)
{
  ...
  for (i=0;i<p->npars;i++)
    if ((!strcmp(p->name[i],par)) && p->active[i] && (p->type[i] == 2))
      return(p->s[i]);
  if ((s = (char **)malloc(sizeof(char *))) == NULL) nomem();
  if ((*s = (char *)malloc(sizeof(char *))) == NULL) nomem();
  **s=(int)0;
  return(s);
}
