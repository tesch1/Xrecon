...
#define SOURCEFILE "recon2D/profile2D.c"
...
void profile2D(struct data *d)
{
...
#ifdef DEBUG
  char function[20];
  strcpy(function,"profile2D"); /* Set function name */
#endif
  ...
  for (d->vol=0;d->vol<d->nvols;d->vol++) { /* loop over "volumes" */
    ...
    for (j=0;j<d->fh.ntraces;j++) { /* loop over traces */
      getmaxtrace1D(d,j);           /* Get coordinates of maximum */
      shifttrace1D(d,j,OPT);        /* Shift FID data for fft */
    }
    d->shift=TRUE;                  /* set flag to 'shifted' */
