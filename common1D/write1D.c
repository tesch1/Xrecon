...
#define SOURCEFILE "common1D/write1D.c"
...
void w1Dtrace(struct data *d,int receiver,int trace,int fileid)
{
  ...
  switch(fileid) {
    case DATA_FILE:
      fp=d->datafp;
      if ((floatdata = (float *)malloc(d->np*sizeof(float))) == NULL) nomem();
      for (k=0;k<dim1;k++) {
    ...
    case PHAS_FILE:
      fp=d->phasfp;
      if ((floatdata = (float *)malloc(dim1*sizeof(float))) == NULL) nomem();
      for (k=0;k<dim1;k++) {
...
#ifdef DEBUG
  char function[20];
  strcpy(function,"wdfh"); /* Set function name */
#endif
...
void openfpw(struct data *d,int fileid)
{
  ...
  char function[20];
  strcpy(function,"openfpw"); /* Set function name */
