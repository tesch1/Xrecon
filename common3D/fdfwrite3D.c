...
#define SOURCEFILE "common3D/fdfwrite3D.c"
...
void w3Dfdfs(struct data *d,int type,int precision,int volindex)
{
  ...
  strcpy(function,"w3Dfdfs"); /* Set function name */
...
void gen3Dfdfs(struct data *d,int mode,char *outdir,int type,int precision,int volindex)
{
  ...
  int offline=FALSE;
  char function[20];
  strcpy(function,"gen3Dfdfs"); /* Set function name */
...
void w3Dfdf(char *filename,struct data *d,int image,int slab,int echo,int receiver,int type,int precision)
{
  ...
  int ix1;
  char function[20];
  strcpy(function,"w3Dfdf"); /* Set function name */
...
void wcomb3Dfdf(char *filename,struct data *d,int image,int slab,int echo,int type,int precision)
{
  ...
  int ix1;
  char function[20];
  strcpy(function,"wcomb3Dfdf"); /* Set function name */
  ...
  switch(precision) {
    ...
    default:
      fprintf(stdout,"\n%s: %s()\n",SOURCEFILE,function);
      fprintf(stdout,"  Invalid 7th argument %s(*,*,*,*,*,*,'precision')\n",function);
      break;
  } /* end precision switch */
...
void gen3Dfdfhdr(struct data *d,int image,int slab,int echo,int receiver,int type,int precision)
{
  ...
  char str[100];
  int cycle,n,id;
  double pro,ppe,ppe2,psi,phi,theta;
  ...
  char **strval;
  char function[20];
  strcpy(function,"gen3Dfdfhdr"); /* Set function name */
...
*/
      sprintf(str,"float  origin[] = {%.6f,%.6f,%.6f};\n",-*val("lro",&d->p)/2,-*val("lpe",&d->p)/2,-*val("lpe2",&d->p)/2);
      strcat(d->fdfhdr,str);
      break;
    case FID: /* FID */
/*
...
*/
  ...
  /* In 2D we include values of psi, phi and theta, but that seems to flip the display
