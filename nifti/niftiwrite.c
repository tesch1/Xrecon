...
#define SOURCEFILE "nifti/niftiwrite.c"
...
void wnifti(struct data *d,int type,int precision,int volindex)
{
  ...
  strcpy(function,"wnifti"); /* Set function name */
...
void writenifti(struct data *d,int type,int precision,int format,int volindex)
{
  ...
  strcpy(function,"writenifti"); /* Set function name */
...
void gennifti(struct data *d,int mode,char *outdir,int type,int precision,int format,int volindex)
{
  ...
  switch (format) {
    ...
    default:
      fprintf(stdout,"\n%s: %s()\n",SOURCEFILE,function);
      fprintf(stdout,"  Invalid 6th argument %s(*,*,*,*,*,'format',*)\n",function);
      return;
      break;
  } /* end format switch */
  createdir(basename);
...
void wniftihdr(char *filebase,struct data *D,int precision,int format,int image)
{
...
#endif
  char function[20];
  strcpy(function,"wniftihdr"); /* Set function name */
    ...
    case NIFTI:
      ...
      if (niftivols>1) {
        ...
        if (timecourse) {                      /* If its a timecourse ... */
          ...
          nfh.dim[4] = 1;                      /* ... the volumes will be in dim[4] ... */
          nfh.dim[4] = (short)niftivols;       /* ... and dim[5] doesn't exist */
        }
      } else {
  ...
  switch(nfh.slice_code) {
    ...
    case 4: fprintf(stdout,"  nfh.slice_code = NIFTI_SLICE_ALT_DEC\n"); break;
      ...
      d32=sintheta*cospsi;
      d33=costheta;
      /* VnmrJ maual states that     [ X ]   [ d11 d21 d31 ] [ x ]
                                     [ Y ] = [ d12 d22 d32 ] [ y ]
                                     [ Z ]   [ d13 d23 d33 ] [ z ]
...
void wniftidata(char *filebase,struct data *d,int receiver,int type,int precision,int format,int newfile)
{
  ...
  int i,j;
  char function[20];
  strcpy(function,"wniftidata"); /* Set function name */
...
void wcombniftidata(char *filebase,struct data *d,int type,int precision,int format,int newfile)
{
  ...
  int i,j,k;
  char function[20];
  strcpy(function,"wcombniftidata"); /* Set function name */
