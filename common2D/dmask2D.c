...
#define SOURCEFILE "common2D/dmask2D.c"
...
void get2Dmask(struct data *d,int mode)
{
  ...
  switch(mode) {
    case MK: /* Mask parameters */
      ...
      if (!strcmp(*sval("masklvlmode",&d->p),"noise")) {
        mask2noise=TRUE;
  ...
  if ((d->mask = (int **)malloc((end-start)*sizeof(int *))) == NULL) nomem();
  for (j=start;j<end;j++)
    if ((d->mask[j] = (int *)malloc(dim2*dim1*sizeof(int))) == NULL) nomem();
...
void fill2Dmask(struct data *d,int mode)
{
  ...
  switch(mode) {
    case MK: /* Mask parameters */
      if (strcmp(*sval("dfill",&d->p),"y")) return;
      start=(int)*val("maskstartslice",&d->p);
      if ((start<0) || (start>ns)) start=1;
      start--;
      end=(int)*val("maskendslice",&d->p);
  ...
  if ((newmask = (int *)malloc(dim2*dim1*sizeof(int))) == NULL) nomem();
