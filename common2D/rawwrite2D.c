...
#define SOURCEFILE "common2D/rawwrite2D.c"
...
int wraw2D(char *filename,struct data *d,int receiver,int slice,int type,int precision)
{
  ...
  int ix1;
  char function[20];
  strcpy(function,"wraw2D"); /* Set function name */

  /* Check that type is valid */
  if ((type != RE) && (type != IM) && (type != MG) && (type != PH) 
    && (type != MK) && (type != SM)) {
    fprintf(stdout,"\n%s: %s()\n",SOURCEFILE,function);
    fprintf(stdout,"  Invalid 5th argument wraw2D(*,*,*,*,'type',*)\n");
    return(1);
  }
  ...
  switch(precision) {
    ...
    default:
      fprintf(stdout,"\n%s: %s()\n",SOURCEFILE,function);
      fprintf(stdout,"  Invalid 6th argument %s(*,*,*,*,*,'precision')\n",function);
      return(1);
      break;
