...
#define SOURCEFILE "common2D/fdfwrite2D.c"
...
void w2Dfdfs(struct data *d,int type,int precision,int volindex)
{
  ...
  strcpy(function,"w2Dfdfs"); /* Set function name */
...
{
  int output=0;
  if (!strcmp(*sval(par,&d->p),"y")) { /* Output selected by par='y' */
    if (d->nr>1) { /* Multiple receivers */
      if (!strcmp(*sval(parIR,&d->p),"y")) /* Individual Receiver output */
        gen2Dfdfs(d,'i',outdir,type,precision,volindex);
...
void gen2Dfdfs(struct data *d,int mode,char *outdir,int type,int precision,int volindex)
{
  ...
  int offline=FALSE;
  char function[20];
  strcpy(function,"gen2Dfdfs"); /* Set function name */
  ...
  if (!validtype(type) || (type == VJ)) {
    fprintf(stdout,"\n%s: %s()\n",SOURCEFILE,function);
    fprintf(stdout,"  Invalid 4th argument %s(*,*,*,'type',*,*)\n",function);
    return;
  }
...
#ifdef DEBUG
  fprintf(stdout,"\n  Block %d (of %d) %s() start slice index = %d, end slice index = %d\n",d->block+1,d->nblocks,function,start,end-1);
  fflush(stdout);
#endif
...
void w2Dfdf(char *filename,struct data *d,int image,int slice,int sliceindex,int echo,int receiver,int type,int precision)
{
  ...
  int ix1,ix2;
  char function[20];
  strcpy(function,"w2Dfdf"); /* Set function name */

  /* Check that type is valid */
  if (!validtype(type) || (type == VJ)) {
    fprintf(stdout,"\n%s: %s()\n",SOURCEFILE,function);
    fprintf(stdout,"  Invalid 8th argument %s(*,*,*,*,*,*,*,'type',*)\n",function);
    return;
  }
  ...
  *null=(char)0;
  ...
  switch(precision) {
    ...
    default:
      fprintf(stdout,"\n%s: %s()\n",SOURCEFILE,function);
      fprintf(stdout,"  Invalid 9th argument %s(*,*,*,*,*,*,*,*,'precision')\n",function);
      break;
...
void wcomb2Dfdf(char *filename,struct data *d,int image,int slice,int sliceindex,int echo,int type,int precision)
{
  ...
  int ix1,ix2;
  char function[20];
  strcpy(function,"wcomb2Dfdf"); /* Set function name */

  /* Check that type is valid */
  if ((type != MG) && (type != RMK)) {
    fprintf(stdout,"\n%s: %s()\n",SOURCEFILE,function);
    fprintf(stdout,"  Invalid 7th argument %s(*,*,*,*,*,*,'type',*)\n",function);
    return;
  }
  ...
  *null=(char)0;
  ...
  switch(precision) {
    ...
    default:
      fprintf(stdout,"\n%s: %s()\n",SOURCEFILE,function);
      fprintf(stdout,"  Invalid 8th argument %s(*,*,*,*,*,*,*,'precision')\n",function);
      break;
  } /* end precision switch */
...
void gen2Dfdfhdr(struct data *d,int image,int slice,int echo,int receiver,int type,int precision)
{
  ...
  char **strval;
  char function[20];
  strcpy(function,"gen2Dfdfhdr"); /* Set function name */

  /* Check that type is valid */
  if (!validtype(type) || (type == VJ)) {
    fprintf(stdout,"\n%s: %s()\n",SOURCEFILE,function);
    fprintf(stdout,"  Invalid 6th argument %s(*,*,*,*,*,'type',*)\n",function);
    return;
  }
  ...
  /* NB addtheta is used to rotate image FOV in ASL test images */
  theta=getelem(d,"theta",image)+getelem(d,"addtheta",image);
...
*/
  ...
  if (arraycheck("image",&d->a)) {
    /* parameter epiref_type may give a quick correction, otherwise count # image=1 values */
    if (spar(d,"epiref_type","single")) arraydim--;
    else if (spar(d,"epiref_type","triple")) arraydim=arraydim-2;
    else if (spar(d,"epiref_type","fulltriple")) arraydim=(arraydim-2)/2;
    else for (i=0;i<nvals("image",&d->p);i++) if (getelem(d,"image",i)<1) arraydim--;
  }
