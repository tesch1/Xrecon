...
#define SOURCEFILE "common2D/tifwrite2D.c"
...
int wtif2D(char *filename,struct data *d,int receiver,int slice,int type,double scale)
{
  ...
  int ix1;
  char function[20];
  strcpy(function,"wtif2D"); /* Set function name */

  /* Check that type is valid */
  if ((type != RE) && (type != IM) && (type != MG) && (type != PH) 
    && (type != MK) && (type != SM)) {
    fprintf(stdout,"\n%s: %s()\n",SOURCEFILE,function);
    fprintf(stdout,"  Invalid 5th argument %s(*,*,*,*,'type',*)\n",function);
    return(1);
  }
  ...
  if ((image = (char *)_TIFFmalloc(dim2*dim1*sizeof(char))) == NULL) nomem();
